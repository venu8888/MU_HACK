...
export default function NearbySync() {
  ...
  const { 
    ...
    connectToBridge, bridgeServerUrl
  } = useAppStore();
          ...
          <Card className="border-amber-500/20 bg-amber-500/5">
            <CardHeader className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <Wifi className={`w-5 h-5 ${bridgeConnected ? 'text-amber-400' : 'text-slate-600'}`} />
                <h3 className="font-bold text-amber-400">Bridge Link (LAN Sync)</h3>
                {bridgeConnected && (
              ...
              <div className="flex gap-2">
                <input 
                  ...
                  defaultValue={bridgeServerUrl?.replace('http://', '')?.replace(':3000', '') || ''}
                  className="bg-slate-900 border border-slate-700 rounded px-2 py-1 text-xs text-white w-28 focus:outline-none focus:border-amber-500/50"
                  onBlur={(e) => connectToBridge(e.target.value)}
                  onKeyDown={(e) => e.key === 'Enter' && connectToBridge(e.target.value)}
                />
                <Button
                  size="sm"
                  variant="outline"
                ...
                >
                  {bridgeConnected ? 'Reset' : 'Link'}
                </Button>
              </div>
            </CardHeader>
            <CardBody>
              <p className="text-[10px] text-slate-500 leading-relaxed italic">
                Connect to a laptop running 'server.js' on this Wi-Fi to sync with all devices automatically. 
                Enter the Laptop's IP above.
              </p>
            </CardBody>
          </Card>
